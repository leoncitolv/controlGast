...
const defaultData={
  ...
  cards:[
    ...
    {name:'Encargos',cutDay:1,payDay:1}
  ],
  apiUrl:''
};
const normalize=d=>{
  ...
  if(!d.cards.some(c=>String(c.name).toLowerCase()==='santander')) d.cards.unshift({name:'Santander',cutDay:15,payDay:5});
  if(!d.cards.some(c=>String(c.name).toLowerCase()==='encargos')) d.cards.push({name:'Encargos',cutDay:1,payDay:1});
  return d;
};
...
function bind(){
 ...
 $('#expenseForm').onsubmit=e=>{e.preventDefault();const d=store.get();const amount=+$('#amount').value||0, paid=+$('#paid').value||0, extraPaid=+$('#extraPaid').value||0; const pagos=[]; if(paid>0)pagos.push({fecha:$('#purchaseDate').value||new Date().toISOString().slice(0,10),monto:paid,nota:'Primer a cuenta'}); if(extraPaid>0)pagos.push({fecha:$('#extraPaidDate').value||new Date().toISOString().slice(0,10),monto:extraPaid,nota:'Abono posterior'}); const totalPaid=paid+extraPaid; d.expenses.push({id:uid(),concept:$('#concept').value,amount,paid:totalPaid,payments:pagos,remaining:Math.max(0,amount-totalPaid),card:$('#cardName').value,purchaseDate:$('#purchaseDate').value||new Date().toISOString().slice(0,10),dueDate:$('#dueDate').value,category:$('#category').value,notes:$('#notes').value,createdAt:new Date().toISOString(),status:(totalPaid>=amount?'paid':'pending')});store.set(d);e.target.reset();$('#paid').value=0;$('#extraPaid').value='';updateRestPreview();suggestDates();};
...
function cardHTML(c,i){const buy=buyFromDay(c);return `<article class="card-chip"><strong>• ${esc(c.name)}</strong><span>• Corte: día ${c.cutDay||'-'} · límite pago: día ${c.payDay||'-'}</span><small>• Mejor comprar desde el día ${buy} para ganar más plazo.</small></article>`;}
...
function updateRestPreview(){const rest=Math.max(0,(+$('#amount').value||0)-(+$('#paid').value||0)-(+($('#extraPaid')?.value||0)));$('#restPreview').textContent=fmt(rest);}
